...
- **v1.1.0** (2026-06-07)：弹窗重构 + 容错修复
  - **视频列表改为独立弹窗**（解决 60+ 视频溢出主窗口的问题）
  - 播放列表/单视频/下载容错：自动跳过无效视频（已删除/地区限制/无权限）
  - 修复 `fetch_video_info` 在播放列表第一个视频失效时的崩溃
  - 清理 GUI 错误信息中的 ANSI 颜色码
  - 下载完成时显示统计信息：`成功 N 个, 跳过 M 个`
